...
class Toolbar
  constructor: (@items, @text, options) ->
    ...
    @threshold = @storage.threshold or @options.thresholds[0]
    @folding()
    @create()
  ...
  go_to_current: ->
    ...
    $("#toolbar_current_item").text @current
    false
  ...
  go_to_alt_current: ->
    ...
    $("html,body").animate scrollTop: pos, 500
    $("#toolbar_current_alt_item").text @alt_current
    false
  ...
  change_threshold: (event) =>
    ...
    @storage.threshold = @threshold = ths[index % ths.length]
    el.text @threshold
    @folding()
    false
