...
class Statistics::Tags < Statistics::Statistics
  ...
  def taggings
    ...
    by_type = select_all("SELECT content_type, COUNT(*) AS cnt FROM taggings,nodes WHERE taggings.node_id=nodes.id AND nodes.public=1 GROUP BY content_type")
  ...
  def taggings_by_year
    ...
    @taggings_by_year = {}
    entries = select_all("SELECT YEAR(CONVERT_TZ(taggings.created_at, 'UTC', 'Europe/Paris')) AS year, content_type, COUNT(*) AS cnt FROM taggings,nodes WHERE taggings.node_id=nodes.id AND nodes.public=1 GROUP BY year, content_type ORDER BY year, content_type")
